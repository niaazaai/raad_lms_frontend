...
const blendModeClass = (mode: React.CSSProperties["mixBlendMode"] | undefined): string => {
  switch (mode) {
    case "normal":
...
const LightPillar = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={cn(
          "absolute inset-0 flex items-center justify-center bg-black/10 text-sm text-muted-foreground",
          blendModeClass(mixBlendMode),
          className,
        )}
      >
        WebGL not supported
  ...
  return (
    <div
      ref={containerRef}
      className={cn("absolute inset-0 h-full w-full", blendModeClass(mixBlendMode), className)}
    />
  );
};
